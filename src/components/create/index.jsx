...
export default class Create extends Component {
  ...
  login() {
    ...
    post("/auth/login/", { email }, false).then(r => {
      if (r.ok) {
        this.setState({ loginPage: 1, showError: false, userId: r.id,
        loading: false });
  ...
  loginWithSSOCode() {
    ...
    post('/auth/token/', {id, code}, false).then(r => {
      if (r.ok) { // user got logged in
        localStorage.setItem('genstemToken', r.tokens.access);
        this.setState({redirect: '/host'})
      } else if (r.err === 'CODE_EXPIRED') {
        this.setState({showError: true, error: 'Koden er ikke riktig, eller ikke gyldig lenger', loading: false});
      }
    });
  }
